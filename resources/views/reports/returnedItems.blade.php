...
<tbody>
    <?php $sumQuantity=0;$sumAmount=0;?>
     ...
     <?php $sumQuantity+=$customer->returns->sum('quantity');$sumAmount+=$customer->returns->sum->amount()?>
    <tr >
        <td >{{$customer->returns->sum('quantity')}}</td>
        <td >{{$customer->returns->sum->amount()}}</td>
        <td > {{$customer->name}}</td>
